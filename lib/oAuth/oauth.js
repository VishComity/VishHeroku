...
oauth.retrieveRefreshToken = function(orgId, res, callBackFunc) {
	var refreshToken=[];
	...
	knex.select('refresh_token')
	    ...
	    .then(function(refTokRows) {
	    	...
	    	callBackFunc(res, refTokRows[0].refresh_token);
	    })
		.catch(function(error) {
		    console.log('ERROR --> '+error);
		  });
...
oauth.retrieveAccessForRefreshToken = function(refreshToken) {
...
oauth.gotoHomePage = function(res, refreshToken){
	console.log('Final  - '+refreshToken);
	if (refreshToken){
		oauth.retrieveAccessForRefreshToken(refreshToken);
	}
};

oauth.retrieveAuthDisplayInfo = function(req, res, app) {
	console.log('Value entered - '+ req);
	oauth.retrieveRefreshToken('00Do0000000KC49EAG', res, oauth.gotoHomePage);
