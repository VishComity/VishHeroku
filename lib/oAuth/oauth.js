...
oauth.retrieveRefreshToken = function(orgId, callBackFunc) {
	var refreshToken=[];
	...
	knex.select('refresh_token')
	    ...
	    .then(function(refTokRows) {
	    	...
	    	callBackFunc(refTokRows[0].refresh_token);
	    })
		.catch(function(error) {
		    console.log('ERROR --> '+error);
		  });
...
oauth.gotoHomePage = function(refreshToken){
...
oauth.retrieveAuthDisplayInfo = function(req, res, app) {
	...
	oauth.retrieveRefreshToken('00Do0000000KC49EAG', oauth.gotoHomePage);
