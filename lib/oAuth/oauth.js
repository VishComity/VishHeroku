...
oauth.retrieveRefreshToken = function(orgId, app, res, callBackFunc) {
	var refreshToken=[];
	...
	knex.select('refresh_token')
	    ...
	    .then(function(refTokRows) {
	    	...
	    	callBackFunc(app, res, refTokRows[0].refresh_token);
	    })
		.catch(function(error) {
		    console.log('ERROR --> '+error);
		  });
...
oauth.retrieveAccessForRefreshToken = function(app, res, refreshToken) {
	console.log('Before calling refreshToken - '+refreshToken);
	connection.getOrg().refreshToken({refresh_token: refreshToken}, function(err, resp){
		if(!err) {
			console.log('Access Token: ' + resp.access_token);
			app.locals.oauthtoken = resp.access_token;
			app.locals.refreshtoken = resp.refresh_token;
...
oauth.gotoHomePage = function(app, res, refreshToken){
	console.log('Final  - '+refreshToken);
	if (refreshToken){
		oauth.retrieveAccessForRefreshToken(app, res, refreshToken);
	}
};

oauth.retrieveAuthDisplayInfo = function(req, res, app) {
	console.log('Value entered - '+ req);
	oauth.retrieveRefreshToken('00Do0000000KC49EAG', app, res, oauth.gotoHomePage);
